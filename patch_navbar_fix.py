...
# Find the exact start of the mute button
target = '<button\n              type="button"\n              className={
av-mute'

if target in content:
    content = content.replace(target, button_str + '            ' + target)
else:
    print("Target not found exactly.")
    # fallback to regex
    content = re.sub(
        r'(<button[^>]*onClick=\{toggleMute\})',
        button_str + r'\n            \1',
        content,
        flags=re.DOTALL
    )
...
print("Toggle check:", content.find("{lang ==="))
